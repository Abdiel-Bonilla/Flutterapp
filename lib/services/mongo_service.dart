...
class MongoService {
  ...
  Future<void> connect() async {
    try {
      _db = await mongo.Db.create(
          'mongodb+srv://abdbon12:abdi03@cluster0.al4iq.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0');
      await _db.open();
  ...
  mongo.Db get db {
    if (!_db.isConnected) {
      throw StateError(
          'No conectado a la base de datos, llama a connect() primero');
    }
    return _db;
  }
  ...
  Future<List<GroupModel>> getGroups() async {
    ...
    print('Coleccion abtenida: $collection');
  ...
  Future<void>updateGroup(GroupModel group) async {
    ...
    await collection.updateOne(
      ...
      mongo.modify.set('name',group.name).set('type',group.type).set('amlbums',group.albums),
    );
  }
